...
export const updateProfile = catchAsyncError(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message: "Profikle Updated Successfully",
  });
});
...
export const delteUser = catchAsyncError(async (req, res, next) => {
  const user = await User.findById(req.params.id);
...
export const delteMyProfile = catchAsyncError(async (req, res, next) => {
  const user = await User.findById(req.user._id);
